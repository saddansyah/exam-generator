...
function OddEvenExam() {
  ...
  const handleAnswer = (userAnswer: "0" | "1") => {
    ...
    const durationFromSessionStart = Math.round((Date.now() - sessionStartTimeRef.current) / 1000);
      ...
      updatedAccumulated.forEach((sess) => {
        for (let i = 0; i < intervalsPerSession; i++) {
          ...
          const answers = sess.answers.filter(
            (a) => a.durationFromSessionStart >= start && a.durationFromSessionStart < end
          );
      ...
      breakTimerRef.current = setInterval(() => {
        setBreakTimeLeft((prev) => {
          if (prev <= 1) {
            ...
            updateSearch({ stage: "testing", currentSession: search.currentSession + 1 });
            return 0;
          }
          return prev - 1;
        });
  ...
  const handleAbort = () => {
    if (confirm("Abort practice test? Your progress for this drill will be lost.")) {
      localStorage.removeItem(TEMP_STORAGE_KEY);
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground uppercase">Session Duration</label>
                    <Select
                      value={search.duration.toString()}
                      onValueChange={(val) => updateSearch({ duration: Number(val) })}
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground uppercase">Total Sessions</label>
                    <Select
                      value={search.sessions.toString()}
                      onValueChange={(val) => updateSearch({ sessions: Number(val) })}
                    >
                      <SelectTrigger>
                ...
                <div className="rounded-lg border bg-muted/30 p-4 flex gap-3 text-xs text-muted-foreground">
                  ...
                  <div className="space-y-1">
                    ...
                    <p>1. An equation like <strong>n + m</strong> is presented.</p>
                    <p>2. If the result is <strong>EVEN</strong>, click <strong className="text-emerald-500 font-bold">0</strong>.</p>
                    <p>3. If the result is <strong>ODD</strong>, click <strong className="text-violet-500 font-bold">1</strong>.</p>
                  </div>
                </div>
              </CardContent>
          ...
          <div className="max-w-md mx-auto text-center">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">Next session starts automatically</p>
              </CardContent>
            ...
            <div className="max-w-md mx-auto text-center">
              <Card>
                <CardHeader className="items-center">
                  ...
                  <CardDescription>Could not locate the practice run results in storage.</CardDescription>
                </CardHeader>
                <CardFooter className="justify-center">
                  <Link to="/exam/odd-even" search={{ stage: "config", duration: search.duration, sessions: search.sessions, currentSession: 1 }}>
        ...
        return (
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="text-center space-y-1">
              ...
              <p className="text-sm text-muted-foreground">Practice report results</p>
            </div>

            <div className="grid grid-cols-3 gap-3">
              <Card>
                <div className="text-center p-3">
                  <span className="text-[10px] font-semibold text-muted-foreground uppercase">Correct Score</span>
                  <div className="mt-1 text-xl font-bold text-emerald-500">
                    {run.totalCorrect}
                    <span className="text-muted-foreground text-sm font-normal">/{run.totalQuestions}</span>
                  </div>
                </div>
              </Card>
              ...
              <Card>
                <div className="text-center p-3">
                  <span className="text-[10px] font-semibold text-muted-foreground uppercase">Accuracy</span>
                  <div className="mt-1 text-xl font-bold text-emerald-500">{run.accuracy}%</div>
                </div>
              </Card>

              <Card>
                <div className="text-center p-3">
                  <span className="text-[10px] font-semibold text-muted-foreground uppercase">Consistency</span>
                  <div className="mt-1 text-xl font-bold text-violet-500">{run.consistency}%</div>
                </div>
              </Card>
            </div>
            ...
            <div className="flex justify-between gap-3 border-t pt-4">
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => updateSearch({ stage: "config" })} className="gap-1.5">
        ...
        <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3">
          <Link to="/" className="flex items-center gap-2 hover:opacity-90">
            ...
            <div>
              ...
              <p className="text-[9px] text-muted-foreground uppercase font-medium">Odd-Even Test Mode</p>
            </div>
          </Link>
        </div>
      </header>
